function SubFunPlotErrorBar(x,y_mean,y_errorbar,PlotColor,PlotMode,varargin)
...
% % % % %    GroupPair.SamplePlot=1; %%%%%%%%%1 plot individual sample; 2 plot red for increase and blue for decrease; 0 No plot individuall sample;
...
% %         plot([x(i) x(i)],y_mean(i)+[-y_errorbar(i) y_errorbar(i)],'color',barColor(i,:));
        ...
        plot(x(i),y_mean(i),'color',barColor(i,:),'linestyle','none','marker',Marker{i},'markersize',4,'markerfacecolor',[1 1 1]);hold on
        plot([x(i) x(i)],y_mean(i)+[-y_errorbar(i) y_errorbar(i)],'color',barColor(i,:),'linewidth',1);
